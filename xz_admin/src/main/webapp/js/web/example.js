...
{
    var url="example/json/addExample";
...
function changePage(opt,functionName)
{
    ...
    window.location.href ="example"+functionName+".html?id="+id;
};
function updateRecord(opt)
{
    var url="example/json/updateExampleById";
...
{
    var url="example/json/getExampleById";
    var data = {id: id};
    ajaxPost(url, data, function (result) {
        if(result==null)back();
...
{
    var id = $(opt).attr("data");
    var url="example/json/delExampleById";
    var data = {id: id};
...
function initTable(opt)
{
    ...
    var url="example/json/findExamplesByPage";
    ...
    ajaxPost(url, data, function (pager) {
        ...
        header = header + '    <tr>';
        //动态数据Start
        header = header + '<th>#ID</th><th>用户名称</th><th>年龄</th><th>状态</th><th>性别</th><th>简介</th><th>头像</th><th>头衔</th>';
        //动态数据End
