...
public class FileUtil {
    ...
    public static String uploadFile(String functionPath, HttpServletRequest request, MultipartFile uploadFile)
        ...
        if (!uploadFile.isEmpty()) {
            ...
            String filename = IdUtil.getUuid() + fileType;
    ...
    public static String writeByteArryToImage(HttpServletRequest request, byte[] imgFile, String fileType) {
        if (imgFile.length != 0) {
            String filename = IdUtil.getUuid() + "." + fileType;
//			String timePath = getTimePath();
            String uploadPath = "/" + "upload" + "/";//统一放在upload下面
            String rootPath = request.getSession().getServletContext().getRealPath("");
            String realPath = rootPath + uploadPath;
            creatFolder(realPath);
            String absolutePath = realPath + filename;
            File file = new File(absolutePath);
            try {
                FileImageOutputStream output = new FileImageOutputStream(file);
                output.write(imgFile);
                output.flush();
                output.close();
    ...
    public static byte[] jpg2Byte(String filePath) {
        byte[] buffer = null;
        byte[] cBuffer = null;
        try {
            File file = new File(filePath);
    ...
    public static void byte2Jpg(byte[] buf, String filePath, String fileName) {
